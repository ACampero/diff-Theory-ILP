...
knowledge_pos = Variable(knowledge_pos.narrow(0, 1, knowledge_pos.size()[0]-1)).cuda()
knowledge_neg = Variable(knowledge_neg.narrow(0, 1, knowledge_neg.size()[0]-1)).cuda()
...
core_rel = Variable(torch.rand(13, num_feat_facts).cuda(), requires_grad=True)
...
rule1 = torch.Tensor([1,0,1,0,1,0]).view(1,-1).cuda()
rule2 = torch.Tensor([0,1,1,0,0,1]).view(1,-1).cuda()
...
for epoch in range(num_iters):
    optimizer.zero_grad()
    facts = torch.cat((core_rel, Variable(torch.ones(13).view(13,1).cuda())), 1)
