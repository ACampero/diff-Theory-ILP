...
for epoch in range(num_iters):
    ...
    for core_indi in range(num_core):
        auxi =  torch.sum(data_aux[core_indi].expand(data_aux[:num_core,:].size()) == data_aux[:num_core,:],1)
        auxi[core_indi] = 0
	...
	#_, indi = torch.max(F.cosine_similarity(targ.view(1,-1).expand(facts[num_core+sparse_core:,:-1].size()),facts[num_core+sparse_core:,:-1]),0)
        #indi=indi.data[0]
        #loss += criterion(facts[num_core+sparse_core+indi,:-1],targ)/(facts[num_core+sparse_core+indi,-1]+epsilon)
