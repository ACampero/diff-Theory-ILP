...
#gumble softmax
#sample from the gumble distribution
def sample_gumbel(shape,eps):
    U = torch.rand(shape)
    U = -torch.log(-torch.log(U+eps)+eps)
    # print(U)
    return U

def gumble_softmax_sample(logits, tau, eps):
    y = logits + sample_gumbel(logits.size(),eps)
    m = torch.nn.Softmax(dim=1)
    return m(y/tau)

def sample_rules(rules_sup, tau, eps):
    '''
    rules_sup must be the tensors with the logits for each predicate in each rule. It's a list of rules where each rule is a matrix,
    each row encoding the distribution for each predicate in the rule, the first row should be the head, second first body and so on
    '''
    rules = []
    for r in rules_sup:
        rules.append(gumble_softmax_sample(r,tau,eps).view(-1))
    rules = torch.stack(rules)
    return rules
...
rules_sup = []
for _ in range(len(rules_str)):
    r = torch.tensor(torch.log(torch.tensor([[1/num_predicates]*num_predicates]*3)+1e-10),requires_grad=True)
    rules_sup.append(r)
tau = 1.0
eps = 1e-20
...
optimizer = torch.optim.Adam([
             ...
             # {'params': [embeddings]},
             {'params': rules_sup, 'lr': learning_rate_rules1}
...
for epoch in range(num_iters):
    ...
    rules = sample_rules(rules_sup,tau,eps)
...
rules_sup = []
for _ in range(len(rules_str)):
   r = tr.tensor(tr.log(tr.tensor([[1/num_predicates]*num_predicates]*3)+1e-10),requires_grad=True, device=device)
   rules_sup.append(r)
